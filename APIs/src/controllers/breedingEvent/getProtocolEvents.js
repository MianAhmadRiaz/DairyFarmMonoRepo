...
const GetProtocolEvents = async (req, res, next) => {
    try {
        ...
        const { count, rows: protocolEventHistory } = await Protocol.findAndCountAll({
            where,
            offset,
            limit,
        ...
        return sendSuccessResponse(res, 200, true, "Protocol Event hsitory fetched successfully.", "breeding-event", responseData);
    } catch (error) {
        next(error);
    }
    return GetProtocolEvents;
};
export default GetProtocolEvents;
