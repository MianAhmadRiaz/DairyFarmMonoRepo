...
const flatten = (alerts: HerdAlerts): FlatAlert[] => {
  const name = (a: HerdAlertItem) => a.name || a.tagName || 'Unknown'
...
const AlertsCard = ({ alerts }: Props) => {
  ...
  return (
    <GlassCard
      ...
      subtitle={total ? `${total} animals need attention` : undefined}
      right={
        total > 0 ? (
