...
const GetHeatEvents = async (req, res, next) => {
    try {
        ...
        const { count, rows: heatEventHistory } = await HeatEvents.findAndCountAll({
            where,
            offset,
            limit,
        ...
        return sendSuccessResponse(res, 200, true, "Heat Event hsitory fetched successfully.", "breeding-event", responseData);
    } catch (error) {
        next(error);
    }
    return GetHeatEvents;
};
export default GetHeatEvents;
