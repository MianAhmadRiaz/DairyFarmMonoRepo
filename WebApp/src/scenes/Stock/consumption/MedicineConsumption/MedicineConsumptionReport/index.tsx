...
const MedicineConsumptionReport: React.FC = () => {
  ...
  const fetchReport = async (start: string, end: string) => {
    try {
      ...
      const response = await fetchFeedCostAnalysis(start, end , medicineCategory.uuid);
  ...
  const handleExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, t('stock.medicineConsumptionReport.sheetName'));
  ...
  return (
    <PageContainer title={t('stock.medicineConsumptionReport.title')} subtitle={t('stock.medicineConsumptionReport.subtitle')}>
                        ...
                        return (
                          ...
                          >
                            ...
                            <TableCell align="right">
                              {(parseFloat(item.total_quantity) * item['item.stockLevel.price']).toFixed(2)}
                            </TableCell>
                          </TableRow>
                        );
                      })}
...
export default MedicineConsumptionReport;
