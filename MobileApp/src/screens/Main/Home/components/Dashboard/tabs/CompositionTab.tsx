...
interface DonutProps {
  slices: { name: string; label: string; value: number; color: string }[]
  centerValue?: string
  centerLabel?: string
}
...
const CompositionTab = ({ refreshKey }: Props) => {
  ...
  const composition = categories
    .map(c => ({
      name: c,
      // Category keys come from the API — fall back to the raw name
      label: t(`main.dashboard.compositionTab.categories.${c}`, c),
  ...
  const pregnancy = [
    {
      name: 'Pregnant',
      label: t('main.dashboard.common.pregnant'),
      value: Number(pregnantPct.toFixed(1)),
      color: COLORS.chartGreen
    },
    {
      name: 'Open',
      label: t('main.dashboard.compositionTab.open'),
      ...
      <GlassCard title={t('main.dashboard.common.herdComposition')}>
        {totalComposition > 0 ? (
          <Donut
            ...
            centerLabel={t('main.dashboard.common.animals')}
          />
        ) : (
          <EmptyState
            title={t('main.dashboard.compositionTab.noAnimals')}
            icon="🐄"
          />
        )}
      </GlassCard>

      <GlassCard title={t('main.dashboard.compositionTab.pregnancyStatus')}>
        {herdInfo?.totalAnimals ? (
          <Donut
            slices={pregnancy}
            centerValue={`${pregnantPct.toFixed(1)}%`}
            centerLabel={t('main.dashboard.compositionTab.pregnantLower')}
          />
        ) : (
          <EmptyState
            title={t('main.dashboard.compositionTab.noPregnancyData')}
            icon="🤰"
          />
        )}
      </GlassCard>
