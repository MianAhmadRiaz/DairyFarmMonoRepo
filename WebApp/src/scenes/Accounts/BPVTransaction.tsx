...
export default function BPVTransaction() {
  ...
  const handleSave = async () => {
    ...
    if (!paymentFrom) {
      setSnackbar({ open: true, message: t('accounts.common.selectPaymentAccount'), severity: 'error' });
      return;
    }
    ...
    try {
      ...
      setSnackbar({ open: true, message: t('accounts.bpvTransaction.saveSuccess'), severity: 'success' });
      handleReset();
    } catch (e) {
  ...
  const handlePrint = () => {
    const printContent = `
      <html>
        <head>
          <title>${t('accounts.bpvTransaction.printTitle')}</title>
          <style>
        ...
        <body>
          <div class="header">
            <h2>${t('accounts.bpvTransaction.title')}</h2>
  ...
  return (
    <PageContainer title={t('accounts.bpvTransaction.title')}>
        {/* Main Card */}
        <Paper elevation={0} sx={{ p: 0, borderRadius: 2, border: `1px solid ${theme.palette.divider}`, bgcolor: 'background.paper' }}>
          ...
          >
            <Typography variant="h6" fontWeight={600} sx={{ display: 'flex', alignItems: 'center' }}>
              ...
              {t('accounts.bpvTransaction.title')}
            </Typography>
