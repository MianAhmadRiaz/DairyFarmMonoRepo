...
const OverviewTab: React.FC = () => {
  ...
  const alertItems = [
    ...(alerts?.pregnancyCheckDue || []).map((a: any) => ({ ...a, kind: 'Pregnancy check due' })),
    ...(alerts?.dryOffDue || []).map((a: any) => ({ ...a, kind: 'Dry-off due' })),
    ...(alerts?.calvingExpected || []).map((a: any) => ({ ...a, kind: 'Calving expected' })),
    ...(alerts?.heatWatch || []).map((a: any) => ({ ...a, kind: 'Heat watch' })),
  ].slice(0, 5);

  return (
    <Grid container spacing={2.5}>
      <Grid item xs={6} sm={4} md={2}>
        <StatTile label="Total Animals" value={herdInfo?.totalAnimals ?? '—'} icon="🐄" delay={0} />
      </Grid>
      <Grid item xs={6} sm={4} md={2}>
        <StatTile label="Milking Cows" value={herdInfo?.milk ?? '—'} icon="🥛" delay={0.05} />
      </Grid>
      <Grid item xs={6} sm={4} md={2}>
        <StatTile label="Pregnancy Rate" value={reproSummary?.pregnancyRate !== undefined ? `${reproSummary.pregnancyRate}%` : `${herdInfo?.pregnantPercentage ?? 0}%`} icon="🤰" delay={0.1} />
      </Grid>
      <Grid item xs={6} sm={4} md={2}>
        <StatTile label="Today's Milk" value={`${Number(milkInfo?.today_total_milk || 0).toFixed(0)} L`} sublabel={`Yesterday: ${Number(milkInfo?.yesterday_total_milk || 0).toFixed(0)} L`} icon="🧴" delay={0.15} />
      </Grid>
      <Grid item xs={6} sm={4} md={2}>
        <StatTile label="Avg / Cow" value={`${Number(milkInfo?.avg_milk_per_cow || 0).toFixed(1)} L`} icon="📊" delay={0.2} />
      </Grid>
      <Grid item xs={6} sm={4} md={2}>
        <StatTile label="Calving Interval" value={reproSummary?.avgCalvingIntervalDays ?? '—'} sublabel="days (herd avg)" icon="🔁" delay={0.25} />
      </Grid>

      <Grid item xs={12} md={8}>
        <GlassCard delay={0.3}>
          <Typography sx={{ fontWeight: 700, mb: 1 }}>30-Day Milk Trend</Typography>
          {milkTrend.length ? (
            <TrendLineChart data={milkTrend} xKey="period" series={[{ dataKey: 'milk', color: '#4cceac', area: true, name: 'Milk (L)' }]} height={240} />
          ) : (
            <EmptyState title="No milk records in this window" icon="📈" />
          )}
        </GlassCard>
      </Grid>
      ...
      <Grid item xs={12} md={4}>
        <GlassCard delay={0.35} sx={{ height: '100%' }}>
          <Typography sx={{ fontWeight: 700, mb: 1.5 }}>Today's Alerts</Typography>
          {alertItems.length ? (
            <Box>
              {alertItems.map((a: any, idx: number) => (
                ...
                >
                  ...
                  <Typography sx={{ fontSize: 11.5, color: theme.palette.text.secondary }}>{a.name || a.tagName || 'Unnamed'}</Typography>
                </Box>
              ))}
            </Box>
          ) : (
            <EmptyState title="No urgent alerts today" icon="✅" />
          )}
        </GlassCard>
      </Grid>
    </Grid>
